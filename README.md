...
- **推奨買い目の成績**：**注目レース TOP3** のうち**軸馬（単勝）の確定オッズで期待値がプラス**
  だったレースだけを対象に、推奨した単勝・3連複・3連単を 100 円均等で買った場合の
  **回収率をホームでグラフ表示**
...
GitHub Actions
 ...
 │    確定した結果と払戻を取得 → 注目レース TOP3 かつ軸馬の確定オッズで期待値プラス
 │    だったレースのみ損益を ledger に記録（それ以外は見送りとして記録しない）
 │    → 結果が取れた全レースは学習モデルに逐次反映 (partial_fit)
 └ bootstrap-train  手動
